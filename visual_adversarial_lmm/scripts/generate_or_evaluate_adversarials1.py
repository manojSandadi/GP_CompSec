...
import sys
import re
sys.path.append("C:/Users/smano/Desktop/Computer Security/visual_adversarial_lmm")
...
from attacks.semantic_attack import apply_semantic_attack
from attacks.region_specific_attack import apply_region_specific_attack
from attacks.universal_pertubation import UniversalAdversarialAttack
...
def generate_adversarials_dataset(model, args):
    ...
    if args.attack_name != "None":
        if args.attack_name == "semantic":
            def attack(images, labels=None):
                # Apply semantic attack directly on PyTorch tensors
                perturbed_images = []
                for img in images:
                    # Convert PyTorch tensor to a NumPy array, apply the attack, and convert back
                    perturbed_img = apply_semantic_attack(img.permute(1, 2, 0).cpu().numpy())
                    perturbed_img = torch.tensor(perturbed_img).permute(2, 0, 1).to(images.device)
                    perturbed_images.append(perturbed_img)
                return torch.stack(perturbed_images)

        elif args.attack_name == "region_specific":
            def attack(images, labels=None):
                try:
                    target_layer = model.visual
                except AttributeError:
                    print("Using alternate target layer...")
                    target_layer = model.backbone  # Replace with an actual layer from the printed architecture

                print(f"Target layer for Grad-CAM: {target_layer}")
                return torch.stack([
                    apply_region_specific_attack(img, model=model, target_layer=target_layer)
                    for img in images
                ])



        elif args.attack_name == "universal":
            def attack(images, labels=None):
                attacker = UniversalAdversarialAttack(model=model, epsilon=0.05, max_iter=100)
                attacker.set_normalization_used(
                    mean=[0.48145466, 0.4578275, 0.40821073], 
                    std=[0.26862954, 0.26130258, 0.27577711]
                )
                universal_delta = attacker.generate(dataloader)
                return attacker.attack(images, universal_delta)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", type=str, default="classification", choices=[
        "classification",
        "retrieval",
        "retrieval_mean",
    ...
    if args.save_image:
        if not args.save_folder:
            sanitized_attack_params = re.sub(r'[{}:"\s]', '_', str(args.attack_params))
            args.save_folder = "adv_datasets/{}/{}/{}/{}_attack_params_{}_{}".format(
                args.dataset,
                args.task,
                args.attack_name,
                model_name,
                sanitized_attack_params,
                args.run_id
        )
    ...
    if args.attack_name:
        if args.attack_params in ['strong', 'normal']:
            args.attack_params = load_yaml_parameters('attacks/config.yaml', args.attack_name).get(args.attack_params, {})
        else:
            try:
                args.attack_params = json.loads(args.attack_params)
            except json.JSONDecodeError:
                raise ValueError("Invalid format for --attack_params. Provide a valid JSON string or use 'strong'/'normal'.")
